...
Both the Kleinwächter EFM 115 and the THUNDERMILL01 are used on measurement cars. The Kleinwächter EFM 115 has lower time resolution compared to the THUNDERMILL01, as illustrated in Figures \ref[figur_efmdatadetail] and \ref[figur_THUNDERMILL01_detail].

The THUNDERMILL01 provides additional angular information by outputting the value of each digitized voltage point on the electrodes during each half-rotation. This feature not only improves time resolution but also avoids averaging values over the entire rotation, which is a limitation of the Kleinwächter EFM 115 and similar electric field mills. Therefore, the THUNDERMILL01 offers a more precise and detailed measurement of the electric field changes.

Both mills were installed on a car and were not grounded. The Kleinwächter EFM 115 measures the gradient of electric potential between the sensor and the car body, capturing the vertical electric field component over a large area and influenced by the car's presence. The THUNDERMILL01, with its superior time resolution and detailed angular data, provides a more accurate representation of electric field dynamics.
...
%\caption/f   Example of raw THUNDERMILL01 data output corresponding to a signal during a half-turn of EFM. The gray mark corresponds to the middle gray mark in Figure \ref[figur_THUNDERMILL01_detail]. Showing sudden change of the voltage on electrodes during the turning of THUNDERMILL01  EFM. This graph at the same time demonstrates the angular sensitivity of THUNDERMILL01.
%\medskip
...
It has been shown that the most noticeable phenomena in VLF recordings (excluding CG return stroke, which according to our measurements occur in less than 10\% of cases) are so-called recoil leaders, sometimes referred to as needles. These are linear objects that cannot be interpreted as a single point. This finding has a significant impact on existing systems attempting to map lightning discharges based on radio localization.
