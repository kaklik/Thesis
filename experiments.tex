...
It has been shown that the most noticeable phenomena in VLF recordings (excluding return stroke, which according to our measurements occur in less than 10\% of cases) are so-called recoil leaders, sometimes referred to as needles. These are linear objects that cannot be interpreted as a single point. This finding has a significant impact on existing systems attempting to map lightning discharges based on radio localization.
