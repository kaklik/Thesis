...
I have participated in the launch of  multiple stratospheric balloons \mycite[j3], the overview of flights is summarized in Table 1 in appendix \ref[FIK_flights]. At the beginning (until the FIK6 flight) the construction of balloon gondolas was designed for specific instruments used for a particular flight. Balloon avionics was therefore built around the chosen detectors from scratch.
...
\secc Atmospheric Radiation Detectors
...
As the measured data show in Fig \ref[R-P_maximum], there is a very noticeable difference between a number of data points measured during the flight upwards and during the descent. This difference is mainly caused by the different values of vertical speeds. In future balloon flights, this limitation is planned to be overcome by a controlled descent, during which the rate of descent can be decreased in some phases of the flight so that it can be more comparable with the speed of the ascent. It could be also seen from Table 1 in appendix \ref[FIK_flights], that the last unsolved issue with stratospheric balloon flights is the safe control of descent. Therefore, in future balloon flights and thunderstorm exploration, there exists a requirement to use the autopilot already contained in TF-ATMON to control descent trajectory.
...
\secc Takeoff technique for thunderstorm observations
...
This system however had a lot of imperfections leading to its abandonment in the measuring car CAR0, where it was replaced by solely manual activation of recording until the trigger was implemented directly into the VLF receiver’s FPGA. See fig. \ref[CRREAT_CAR0_diagram] for an overview.
...
To demonstrate the actual detection of thunderstorm-related radiation events, Figure \ref[GEODOS01_polednik] shows an event captured by the GEODOS01 instrument located statically on the Polednik tower. There was captured a new type of radiation phenomena with could not be classified clearly as TGE or TGF. The detailed description of this significant result is out of the scope of this thesis but is published in \mycite[j5].
...
As a bonus, THUNDERMILL01 provides additional angular information by outputting the value of each digitized voltage point on the electrodes during each half-rotation. This feature not only improves time resolution but also avoids averaging values over the entire rotation, which is a limitation of the Kleinwächter EFM 115 and similar electric field mills. Therefore, the THUNDERMILL01 offers a more precise and detailed measurement of the electric field changes. The angular sensitivity is explained in \mycite[c4].
...
\caption/f Example of raw THUNDERMILL01 data output corresponding to a signal during a half-turn of EFM. The gray mark corresponds to the middle gray mark in Figure \ref[figur_THUNDERMILL01_detail]. Showing sudden change of the voltage on electrodes during the turning of \nobreak{THUNDERMILL01} EFM.
\medskip
