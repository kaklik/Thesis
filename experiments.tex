...
Figure \ref[FIK-6_telemetry] demonstrates the importance of telemetry data from the TF-ATMON system during measurement processing. The graphs show an increase in the response of the silicon ionizing radiation detector at the times of take-off, burst, and landing when there was a rapid increase in mechanical stress. The effect is caused by the microphone effect of the silicon detector circuit. The similar situation is observable during the balloon descent where can be seen there are considerable vibrations, rotations and rapid changes in acceleration of gondola. The increase in relative humidity between radiation detectors is also observable in fig \ref[FIK-6_RAW_data]. All of this conditions may affect the atmospheric observations for some types of instruments, they have to be compensated.  Due to the telemetric system \iid TF-ATMON that compensation becomes possible. TF-ATMON enables data recording, pre-flight instruments check, and their monitoring during flight. At the same time thanks to the availability of different communication interfaces to the avionics, the use of various payload detectors integration is simplified.
...
As the measured data show in Fig \ref[R-P_maximum], there is a very noticeable difference between a number of data points measured during the flight upwards and during the descent. This difference is mainly caused by the different values of vertical speeds. In future balloon flights, this limitation is planned to be overcome by a controlled descent, during which the rate of descent can be decreased in some phases of the flight so that it can be more comparable with the speed of the ascent. It could be also seen from Table 1 in appendix \ref[FIK_flights], the last unsolved issue with the stratospheric balloon flights is the safe control of descent. Therefore, in future balloon flights and thunderstorm exploration, there exists a requirement to use the autopilot already contained in TF-ATMON to control descent trajectory.
...
Another unresolved challenge for use of the balloons from point of thunderstorm research is the difficulty of launching them in strong winds, which are nearly always present near storms, especially at times when the measurements are most critical. See the Figure \ref[balloon_wind_takeoff] for that situation.
...
I have identified, designed and realized a type of aircraft — a unmanned gyrocopter — that is capable of flying under adverse weather conditions. This aircraft can carry suitable measuring instruments, such as a semiconductor ionizing radiation detector and an electric field mill for mapping the charge structure. Unfortunately, current legislation does not effectively permit to conduct practical measurements during thunderstorms.

\sec Ground level measurements

Within the context of the CRREAT project, the term {\em ground level measurements} encompasses a broader scope, including measurements based on passive and stationary high-altitude observatories. My involvement in these stationary measurements was minimal, primarily relating to some aspects of electronics design. However, I consider it useful to mention the existence of these measurements here because that observations occasionally uses identical equipment delivered from mobile instrumentation described, which allows for the extrapolation of findings from the stationary observation’s data to active ground measurements conducted using measurement vehicles, on where my primary work was focused.
...
To demonstrate the actual detection of thunderstorm related radiation event, the Figure \ref[GEODOS01_polednik] shows an event captured by GEODOS01 instrument located statically on Polednik tower. There was captured a new type of radiation phenomena with could not be classified clearly to TGE or TGF. The detailed description of this significant result is out of the scope of this thesis, but is published in \mycite[j5].
...
Both the Kleinwächter EFM 115 and the THUNDERMILL01 are used on measurement car. The Kleinwächter EFM 115 has lower time resolution compared to the THUNDERMILL01, as illustrated in Figures \ref[figur_efmdatadetail] and \ref[figur_THUNDERMILL01_detail].
...
\caption/f   Detail of the yellow marked section from Figure \ref[figur_THUNDERMILL01_data], alighned with a light curve from Chronos all-sky high-speed camera recording.  Note the significantly higher time resolution of the THUNDERMILL01 data in comparison with \ref[figur_efmdatadetail]. The blue series is THUNDERMILL01's signal amplitude corresponding to 110°.
\medskip
...
%\caption/f   Example of raw THUNDERMILL01 data output corresponding to a signal during a half-turn of EFM. The gray mark corresponds to the middle gray mark in Figure \ref[figur_THUNDERMILL01_detail]. Showing sudden change of the voltage on electrodes during the turning of THUNDERMILL01  EFM. This graph at sametime demonstrate the angular sensitivity of THUNDERMILL01.
%\medskip
