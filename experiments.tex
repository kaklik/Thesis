...
The ground-level observation attempts lead to insights such as the geometry of lightning event is composed mainly from flat and large objects. That situation implicated the possibility of lightning initiation process over large areas, and at the same time the important observation that ionizing radiation does not radiate strictly downward.

It has been shown that the most noticeable phenomena in VLF recordings (excluding return stroke, which according to our measurements occur in less than 10% of cases) are so-called recoil leaders, sometimes referred to as needles. These are linear objects that cannot be interpreted as a single point. This finding has a significant impact on existing systems attempting to map lightning discharges based on radio localization. Additionally, it has been shown that at high frequencies, the negative ends of lightning discharges radiate. Thus, the UHF receiver only gives us an image of the negative end of the discharge.
