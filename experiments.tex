...
\caption/f   Measuring CAR2 equipped with a roof platform used to carry and takeoff the TF-G2 autogyro at the thunderstorm site. The rotor fixing is the black arm on the left. The gray tubes are oriented vertically during takeoff and weights inside are used to prerotate the rotor to minimum RPMs. The embedded video file: {\em TF-G2_autogyro_takeoff.mp4} shows the takeoff process. Please click on the Figure to play the video, alternatively the file could be extracted from the PDF or replayed on the web\urlnote{https://youtu.be/-Ga4DO3h_1c}.  
\endinsert
...
\caption/f   The embedded video file: {\em 1627302288.9546976.mp4} shows lightning together with leaders and recoil leaders. (Video plays after click only in some PDF viewers)
\endinsert
