\sec Objectives Formulation

The objectives of this dissertation adopt a physical, episode-based definition of a lightning event (first detected activity to last detected activity across modalities), rather than an algorithmic GLM/LIS cluster definition. This directly addresses the underestimation bias in satellite-only durations and extents. Therefore, clarify unresolved phenomena related to thunderstorms, particularly the initiation and development of lightning, reviewed in Section~\ref[problem_definition]. 

While technical standards, such as IEC 62305, have been developed based on applied observations using the mentioned instruments, their focus is on defining parameters for protecting technical devices against lightning currents. These standards simplify the modeling of lightning currents to address the needs of anthropogenic terrestrial objects, which are inherently influenced by artificial factors.

In contrast, the scope of this dissertation is firmly rooted in fundamental research, aiming to study lightning discharges in their natural state. The focus is on understanding the intrinsic properties of lightning phenomena without interference from man-made structures, thereby contributing to a deeper scientific understanding of these processes.
...
\begitems \style n
* {\bf Implementation of Lightning Detection Apparatus}: Design and deploy a mobile experimental apparatus capable of scientific-grade radio detection and localization of lightning events and ionizing radiation, considering the expected spatial scales. Collect data using ionizing radiation detectors, radio receivers, high-speed cameras, and ground-based electric field measurements.
* {\bf Enhance understanding of Spatial and Temporal Characteristics of Lightning Discharges}: Enhance visualization methods and improve measurement apparatuses for detecting lightning and ionizing radiation (as described in Section~\ref[problem_definition]). Study the propagation of lightning, emitted radio signals, and associated electric fields, and reflect the physical principles of lightning development and radio emissions.
* {\bf Enhancement of Measurement Techniques with in-situ atmospheric monitoring}: Integrate new instruments, such as stratospheric balloons and unmanned aerial vehicles (UAVs), for in-situ measurements of electric fields and ionizing radiation. Employ new electric field mills and semiconductor-based ionizing radiation detectors integrated into UAVs to explore potential insights into lightning initiation and the locations of ionizing radiation generation within clouds.
\enditems
...
\sec[goals] Thesis Goals

From a top-level perspective, the overall goal of this thesis is to advance the understanding of lightning phenomena and associated atmospheric radiation. 
Current understanding of lightning phenomena, particularly its spatial and temporal dynamics and its relationship with high-energy atmospheric radiation, is based on observational methods that may oversimplify its complexity. Existing detection networks often reduce lightning to localized impulse clusters, potentially missing crucial information about its full extent, initiation phases, and energy transfer mechanisms. A more detailed, high-resolution, and multi-instrument approach is required to unravel these complexities, leading to improved lightning forecasting, safety protocols, and a deeper comprehension of atmospheric electricity.

The field of lightning research faces several challenges, not least the lack of consensus, even within Europe, regarding the principles of so-called “active lightning rods.” Corresponding technical standards remain inconsistent, reflecting broader disagreements and ongoing debates rather than an established consensus. Research in this area is therefore both relevant and necessary.

Studying lightning is inherently challenging due to reliance on technical approximations in commonly used methods, such as artificial lightning current generators or tests initiated by grounded rocket launches. Measurements conducted on artificial structures, such as telecommunications towers, often result in lightning discharges with characteristics that differ significantly from natural lightning phenomena. This highlights the importance of in situ measurements aimed at capturing the intrinsic behavior of natural lightning discharges.

Building on these observations, the following scientific assumptions, extracted from the literature review, provide the foundation for this research. While they represent the current understanding in the field, this dissertation critically examines and tests these assumptions to refine the knowledge of thunderstorm phenomena:
...
Considering what has just been said, we define three main goals of this thesis as described in the following three subsections. 

\secc Characterize the true spatial extent and temporal duration of individual lightning events, challenging existing models and detection paradigms

  Conventional wisdom often estimates lightning events to be short-lived (tens to hundreds of milliseconds) and spatially confined. If these assumptions are incorrect, current theories of lightning physics and safety are fundamentally flawed. Optical and radio observations are needed to accurately measure the start, progression, and end of lightning events, to reveal actual durations and geographical footprints. This re-evaluation is critical for developing more accurate physical models.

Unfortunately, existing lightning observation systems are often geographically fixed or lack the comprehensive suite of sensors needed to capture both the optical and electromagnetic signatures of lightning simultaneously with associated high-energy radiation. To overcome limitations in spatial coverage and sensor diversity, a mobile platform equipped with advanced optical (high-speed cameras), radio frequency (VLF/UHF SDR-based receivers), and ionizing radiation detectors is essential. This integrated approach will allow for correlated data acquisition across different physical domains.

\def\odcomm#1{{\Red #1}}
\def\odchange#1{{\Green #1}}

\secc Investigate lightning and its potential role as an atmospheric ionizing radiation source, particularly through advanced in-situ electric field measurements.
...
\secc  To develop a data processing algorithm capable of visualizing non-point-like structures of lightning discharges

  Displaying lightning as single points on maps, as is common in many lightning detection networks, intuitively misrepresents a phenomenon that is inherentla spatially extended. This oversimplification hinders scientific analysis and public understanding. New algorithms are required to transform raw data (e.g., VLF signals) into visual representations that more accurately depict the three-dimensional, dynamic, and branching nature of lightning channels, thereby improving both scientific interpretation and practical applications like mapping.

 As examples, there could be mentioned the Vaisala lightning sensors \cite[vaisala],  WWLLN \cite[https://doi.org/10.1029/2021RS007293], and LINET \cite[Betz2009].
...
\sec[literature_review] Literature Review: Spatiotemporal Scales of Lightning Discharges
...
\secc Classic RF and field-change studies

Le Vine’s NASA Technical Memorandum reported that lightning RF emission (3–300 MHz) appears as trains of discrete pulses whose time histories differ systematically between cloud-to-ground and intracloud flashes; “mixed” cases reflect breakdown phases preceding stepped leaders. The work emphasized frequency-independent patterns across the monitored band and highlighted the feasibility of studying flash duration and intra-flash pulse statistics—but did not deliver population-scale timing distributions.
...
\secc Space-based optical mapping 

Modern satellite imagers (OTD, LIS, and especially GOES-R GLM) revolutionized lightning mapping with hemispheric views at high frame rates. However, what the instrument calls a “flash” is a product of clustering choices in the ground system: GLM defines a flash as a set of optical groups separated by ≤ 330 ms in time and ≤ 16.5 km in space; exceed either threshold and the activity is split into multiple flashes. These thresholds are engineering parameters, not physical boundaries of a single electrical event. Similar hierarchical clustering exists for LIS/OTD. 

Initial GLM climatologies report broad tendencies—e.g., longer and larger flashes over ocean on average, with typical GLM mean durations of a few hundred milliseconds depending on regime—but again, population medians and robust scale metrics are sparse and sensitive to detection efficiency and clustering. 
Recent assessments continue to show that GLM’s performance and the resulting duration statistics vary with storm type, viewing geometry, background, and algorithm tuning. 

Despite the existence of these advanced devices, the phenomena described in section \ref[problem_definition] have not yet been fully explained, and the relationship between lightning discharges, ionizing radiation, and electromagnetic signals is not fully understood.



\secc Known records but not statistics

Spaceborne imagers have detected spectacular “megaflashes” that stretch our intuition: the distance record now stands at 829 km (22 Oct 2017, Great Plains; duration ~7.4 s), surpassing the 768 km 2020 event; the duration record remains 17.1 s (18 Jun 2020, Uruguay–Argentina). These records reflect what satellites can see over immense mesoscale convective systems and are invaluable for bounding physics and hazards. 

However, extremes are not design points. Instrumentation and derived models for radiation studies need central tendencies (median duration/area/length) and distributional spread—quantities that the literature seldom reports consistently. This gap is partly historical (classic RF campaigns weren’t scaled for population statistics) and partly methodological (GLM/LIS clustering truncates long, sparse episodes and fragments sprawling flashes).

In the literature is quite common to see statements that “typical flashes last up to ~350 ms.” In practice, such values often conflate physical duration with an algorithmic timeout: GLM’s 330 ms inter-group limit is a clustering parameter, not a physical limit on flash lifetimes, and can bias published “duration” toward sub-second values. Comprehensive reviews of lightning physics emphasize that key aspects remain poorly constrained, reinforcing the need for observational definitions aligned with physical episodes rather than processing thresholds. 

For atmospheric ionizing radiation (e.g., glows, TGFs, photonuclear signatures) and for initiation/leader physics, the true time duration the discharge event remains unclear. Recent optical and multi-sensor studies show subcomponents (e.g., continuing currents) with characteristic timescales from tens to hundreds of milliseconds, but these are sub-flash features within longer electrical episodes. 

On the basis of applied observations conducted using existing instruments, the IEC 62305 standard was developed. This standard provides a simplified model of lightning current, focusing on defining parameters for technical devices that must withstand lightning strikes. Its scope is limited to lightning events involving anthropogenic terrestrial objects, which are inherently influenced by artificial factors. In contrast, this dissertation focuses on studying lightning discharges from the perspective of fundamental research, aiming to observe their natural characteristics uninfluenced by man-made objects.
